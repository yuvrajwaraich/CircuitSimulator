...
public class InputOutput : MonoBehaviour
{
    ...
    private GameObject getIODevice(Vector2 mousePos)
    {
        ...
        if (hit.collider != null && hit.collider.gameObject.name != "AND Gate(Clone)" && hit.collider.gameObject.name != "NOT Gate(Clone)" && hit.collider.gameObject.transform.parent.gameObject.name == "IODevice")
        {
