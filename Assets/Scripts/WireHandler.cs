...
public class WireHandler : MonoBehaviour
{
    ...
    public void UpdateLine(GameObject line, GameObject lineEnd, Vector2 mousePos)
    {
        ...
        if (currLineRend != null)
        {
            ...
            if (lineEnd != null && !(lineEnd.GetComponent<InputOutput>().connected) && !(lineEnd.GetComponent<InputOutput>().canOutput) && lineEnd.transform.position != currStart.transform.position)
            {
